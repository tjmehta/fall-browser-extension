...
Tab.prototype.favicon = function () {
  ...
  if (exists(this.faviconUrl) || this.fetchingFavicon) { return this.faviconUrl; }
...
Tab.prototype.fetchBetterFavicon = function () {
  ...
  function updateFavicon (err, faviconUrl) {
    // ignoring errs above..
    self.fetchingFavicon = false;
    if (faviconUrl && self.faviconUrl !== faviconUrl) {
      self.setFaviconUrl(faviconUrl);
    }
    else {
      self.setFaviconUrl('');
    }
...
Tab.prototype.setFaviconUrl = function (faviconUrl) {
  this.faviconUrl = faviconUrl;
  this.emit('change', this, { key: 'favicon', value: this.faviconUrl });
};
