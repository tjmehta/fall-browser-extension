...
Tab.prototype.fetchBetterFavicon = function () {
  ...
  xhr.get(this.url, 'document', function (err, res) {
    ...
    if (xml) {
      var appleIcons = xml.querySelectorAll('link[rel="apple-touch-icon"]');
      console.log('appleIcons', appleIcons);
      var largest = last(appleIcons); // last tends to be largest..
      if (largest) {
        xhr.head(largest.href, function (err, res) {
          console.log('xhr.head(largest.href)', largest, arguments);
          if (err) { return console.log(err); }
          if (res.status >= 400) {
            return console.log(largest.href, 'responded', res.status);
          }
          self.faviconUrl = largest.href;
